...
from model.bakery import createBakeryModel, updateBakeryModel #! importฟังก์ชัน Create และ Update เข้าตัวตารางbakery 
...
#*******************************************************************************************************
@app.get("/")  #!อ่านข้อมูลในdb แล้วreturnออกมาหากสามารถหาเจอได้ status code 200 คือ ปกติ
def index():
    return JSONResponse(content={"message": "Bakery Info"}, status_code=200)

#*******************************************************************************************************
@app.get("/bakerys") #!อ่านข้อมูลในdb มีการsortโดยเรียงmenu_typeตามตัวอักษร A-Z (ASD), Z-A (DSC)
def get_bakerys(
    ...
    try:
        result = mongo_db.find(order,sort_by)  #TODO: sort menu_type : A-Z (ASD), Z-A (DSC)
    except:
        raise HTTPException(status_code=500, detail="Something went wrong !!") #!returnออกมาหากไม่สามารถหาเจอ status code 500 ให้แสดงใน terminal ว่า  Something went wrong !!
...
def get_bakerys_by_id(bakerys_id: str = Path(None, min_length=10, max_length=10)):#!ใส่IDอะไรก็ได้ไม่เกิน 10และไม่ต่ำกว่า10
    try:
        result = mongo_db.find_one(bakerys_id) #!find_oneอันเดียว คือส่ง bakerys_id ที่ส่งมา (ควรมีไอดีเพียงอันเดียว)
    except:
        raise HTTPException(status_code=500, detail="Something went wrong !!") #!500 API Error

    if result is None:
        raise HTTPException(status_code=404, detail="Bakery Id not found !!") #!404 คนคีย์IDผิด

    return JSONResponse( 
        content={"status": "OK", "data": result}, #!returnออกมาหากสามารถหาเจอได้ status code 200 ให้แสดงใน terminal คือ ปกติ
        status_code=200,
    )
...
def delete_bakery_by_id(bakery_id: str = Path(None, min_length=10, max_length=10)): #!ใส่IDอะไรก็ได้ไม่เกิน 10และไม่ต่ำกว่า10
    ...
    if deleted_count == 0: #ถ้าลบdelcount = 0 เอาไอดีมาผิดจะบอกว่าไม่มีนะจีะ
        raise HTTPException(
            status_code=404, detail=f"Bakery Id: {deleted_bakery_id} is not Delete"  #!404 คนคีย์IDผิด
        )
